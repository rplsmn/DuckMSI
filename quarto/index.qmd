...
viewof status = {
  const span = html`<span class="status-indicator"><span style="color: #28a745; font-size: 14px;">✓</span> DuckDB Ready</span>`;
  return span;
}
```
...
// Capture SQL query when button is clicked
sqlToExecute = {
  executeClicked; // Trigger on button click
  return sqlQuery; // Capture current value
}
```
:::
